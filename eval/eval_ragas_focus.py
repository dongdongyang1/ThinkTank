...
def load_dataset() -> List[Dict]:
    ...
    # 过滤：只保留指定类型
    filtered = [s for s in data if s.get("question_type") in FOCUS_TYPES]
    
    print(f"加载测试集: 全部 {len(data)} 条，过滤后 {len(filtered)} 条")
    print(f"过滤类型: {FOCUS_TYPES}")
    return filtered
...
def save_results(df, overall, by_type, by_difficulty, by_retrieval, retrieval_count):
    ...
    json_path = os.path.join(OUTPUT_DIR, f"ragas_report_focus_{timestamp}.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    print(f"JSON 报告: {json_path}")

    # 2. CSV 明细
    csv_path = os.path.join(OUTPUT_DIR, f"ragas_details_focus_{timestamp}.csv")
    df.to_csv(csv_path, index=False, encoding="utf-8-sig")
    print(f"CSV 明细: {csv_path}")

    # 3. HTML 报告
    html_path = os.path.join(OUTPUT_DIR, f"ragas_report_focus_{timestamp}.html")
...
<head>
    ...
    <style>
        ...
        .badge {{ background: #ff9800; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px; margin-left: 10px; }}
    </style>
</head>
<body>
    <div class="container">
        <h1>RAGAS 快速评估报告 <span class="badge">只跑重点类型</span></h1>
        <div class="meta">
            <p>评估时间: {report['timestamp']} | 样本数: {report['total_samples']} | Judge 模型: {report['judge_model']}</p>
            <p>评估模式: {report.get('eval_mode', '')}</p>
        </div>
