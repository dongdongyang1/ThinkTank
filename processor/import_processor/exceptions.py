...
# 调用父类Exception的构造函数，把 message 存进父类内部变量。
# super().__str__() 拿到的只是父类 Exception 储存的 message
# 原生 Exception 本身就会保存错误消息，str(异常实例) 默认会输出这条 message
