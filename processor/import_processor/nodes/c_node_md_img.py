...
class NodeMDImg(BaseNode):
    ...
    def process(self, state: ImportGraphState):
        ...
        3. 调用多模态大模型为图片生成内容摘要
        4. 将图片上传到Minio，替换MD中本地图片路径为MiniIO访问URL，并填充图片摘要
        5. 备份原MD文件，保存处理后的新MD文件并更新状态
        :param state:md_path,md_content
        :return: new_md_path,new_md_content
        """
    ...
    def _step_2_scan_images(self, md_content:str, images_dir:Path):
        """
        ...
        # 2. 遍历图片文件夹
        #os.listdir(文件夹路径)：列出指定文件夹内所有文件、文件夹名称，返回一个字符串列表。
        for image_file in os.listdir(images_dir):

            # 2.1 过滤无效后缀
            #splitext：把「文件路径」切割成 文件名 + 后缀扩展名,返回是一个二元元组 (主体文件名, 后缀)
            ...
            # 2.2 组装图片完整路径并转换成字符串
            #老版 Python库、OpenCV、Pillow、os 接口、文件写入接口不支持直接传入Path对象，只接收字符串路径
    ...
    def _apply_api_rate_limit(
            self,
            request_times : Deque[float],  #双端队列对象
            max_requests : int,
            ...
            messages = [
                {
                    ...
                    "content":[
                        {
                            "type":"text",
                            "text":f"""这是"{root_folder}"文件中的一张图片，图片上文部分为"{image_content[0]}"，
                            下文部分为"{image_content[1]}"，请用中文简要总结这张图片的内容，用于 Markdown 图片标题。"""
                        },
                        {
                            "type":"image_url",
                            "image_url":{
    ...
    def _step_4_upload_and_replace(self,doc_stem:str,target_images:List[Tuple[str,str,Tuple[str,str]]],
                                   summaries:Dict[str,str],md_content):
        """
    ...
    def _clean_minio_directory(self, minio_client:Minio, prefix:str)->None:
        ...
        try:
            ...
            #构造删除列表
            # MinIO SDK 专门用于批量删除接口的结构体，批量删除接口要求传入一组DeleteObject对象，不能直接传文件名字符串。
    ...
    def _upload_to_minio(self, minio_client, img_path, object_name):
        """
    ...
    def _process_md_file(self, md_content:str, image_info:Dict[str,Tuple[str,str]])->str:
        ...
        :return: 替换后的新MD内容
        """
        self.logger.info(f"替换后MD长度：{len(md_content)}，替换图片数量：{len(image_info)}")
        #遍历 image_info字典的每一页：key=图片文件名，value=（摘要，新url）
