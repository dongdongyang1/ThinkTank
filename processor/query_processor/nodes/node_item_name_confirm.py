...
class NodeItemNameConfirm(NodeBase):
    """
    节点功能：确认用户问题中的核心商品名称
    把用户 “自然、模糊的原始问题” 转化为 “精准、适配检索规则的查询语句”
    """

    # 覆盖基类的 name 属性，标识节点名称
    name : str = "node_item_nameconfirm"
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        # 3. 用户初始信息保存
        message_id = save_chat_messages(session_id,"user",original_query)
    ...
    """
        try:
            ...
            # 3. 处理和动态拼接提示词
            # 提示词中为了把大括号当作 “普通字符” 保留下来，用双大括号 {{ 表示普通的左大括号 {，双大括号 }} 表示普通的右大括号 }。
            ...
            #过滤空字符串
            result["item_names"] = [n for n in result["item_names"] if n]
    ...
    """
        ...
        for i in range(len(item_names)):
            try:
                # 从批量生成的向量结果中，取出当前商品名对应的稠密向量
                dense_vector = embeddings.get("dense")[i]

                # 从批量生成的向量结果中，取出当前商品名对应的稀疏向量
                ...
                search_res = hybrid_search(
                    ...
                    rank_weights=(0.8,0.2),  # 稠/稀疏向量评分权重配比（和为1最佳）
                    limit=5,  # 最终返回Top5匹配结果
                    norm_score=True,  # 开启评分归一化，统一评分量级为0-1 商品名+对应相似度评分
    ...
    def _step_6_align_item_names(self, query_results)->dict:
        ...
        for res in query_results:
            ...
            # 获取匹配的商品名，无就获取空列表
            matchs = res.get("matches",[]) or []
            # 若无匹配结果，直接跳过当前商品名的对齐
            if not matchs:
                continue

            # 筛选高置信度匹配结果：评分>0.85
            high = [m for m in matchs if m.get("score",0)>0.85]

            # 筛选中置信度匹配结果：评分≥0.6（仅高置信度为空时生效）
            mid = [m for m in matchs if 0.6<=m.get("score",0)<=0.85]
    ...
    def _step_7_check_confirmation(self, state, align_result, history):
        ...
        if options:
            ...
            # 构造向用户确认的提示语
            answer = f"您是想问以下哪个产品：{options_str}?请确认一下型号"
            state["answer"] = answer
            state["item_names"] = []
            return state
    ...
    def _step_8_write_history(self, state, session_id, rewritten_query, message_id):
        ...
        if state.get("answer"):
            save_chat_messages(
                ...
                text=state.get("answer"),# 消息内容：向用户确认的提示语/无结果提示语
