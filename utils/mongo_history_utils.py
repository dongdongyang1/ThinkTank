...
def update_message_item_names(ids: List[str], item_names: List[str]) -> int:
    ...
    try:
        ...
        result = mongo_tool.chat_message.update_many(
            {"_id": {"$in": object_ids}},  # 条件：键在指定的ID列表中
...
if __name__ == "__main__":
    ...
    sid = "000015_hybrid"
    # 1. 写入用户消息
    save_chat_messages(sid, "user", "你好 (Hybrid)")
    # 2. 写入助手回复
    save_chat_messages(sid, "assistant", "你好！我是基于原生 Mongo + LangChain 对象的助手。")
    # 3. 写入带关联商品的用户消息
    save_chat_messages(sid, "user", "这个万用表怎么换电池？", item_names=["混合万用表"])
